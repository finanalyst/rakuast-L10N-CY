...
repeat {
    ...
    given $choice {
        when "dragon" {
            say "which is 'draig' in Welsh"
        }
        when any(@bad) {
            say "wash your mouth with soap"
        }
        when IntStr {
            say "which evaluates to an integer ", $choice
        }
        when RatStr {
            say "which evaluates to a rational number ", $choice
        }
        default {
            say "which does not evaluate to a number "
        }
    }
